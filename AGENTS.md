...
# Frontend
The frontend is a Vue.js application in `frontend/index.html` (with caching implemented via a service worker in `frontend/sw.js`). The `frontend/` directory is the default for GitHub pages, which is why this repository is structured that way, and the `index.html` file is a single file defining styles, scripts, and content because it was started from within a chat session in Gemini, where the Canvas feature prioritizes single HTML files for simplified rendering. In the future, that file may be separated into content, scripts, and styling for easier maintenance and modularity.
